...
def visualizza_sezione(request, pk):
	...
	discussioni_sezione = Discussione.objects.filter(sezione_apparteneza=sezione).order_by('-data_creazione')
...
def crea_discussione(request, pk):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			discussione.sezione_apparteneza = sezione
			discussione.autore_discuzione = request.user
			discussione.save()
